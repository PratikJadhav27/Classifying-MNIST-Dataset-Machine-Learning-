...
# Train the model
sgd_model.fit(x_train_threshold_flattened, y_train)

# Test the model
y_pred = sgd_model.predict(x_test_threshold_flattened)
thres_accuracy = accuracy_score(y_test, y_pred)

print("SVM on TEST DATASET")

print(f"Thresholded Accuracy on test set: {thres_accuracy:.4f}")
...
sgd_model.fit(x_train_bound_box_flattened, y_train)

y_pred = sgd_model.predict(x_test_bound_box_flattened)
bb_accuracy = accuracy_score(y_test, y_pred)

print(f"Bound Box Accuracy on test set: {bb_accuracy:.4f}")
...
sgd_model.fit(x_train_bound_box_streched_flattened, y_train)

y_pred = sgd_model.predict(x_test_bound_box_streched_flattened)
bbs_accuracy = accuracy_score(y_test, y_pred)

print(f"Bound Box Streched Accuracy on test set: {bbs_accuracy:.4f}")
...
sgd_model.fit(x_train_bound_box_streched_threshold_flattened, y_train)

y_pred = sgd_model.predict(x_test_bound_box_streched_threshold_flattened)

bbs_thres_accuracy = accuracy_score(y_test, y_pred)

print(f"Bound Box Streched tresholded Accuracy on test set: {bbs_thres_accuracy:.4f}")
# Testing on Train data
### SVM on threshold dataset.

sgd_model.fit(x_train_threshold_flattened, y_train)

y_pred = sgd_model.predict(x_train_threshold_flattened)
thres_accuracy = accuracy_score(y_train, y_pred)

print("SVM on TRAIN DATASET")

print(f"Thresholded Accuracy on train set: {thres_accuracy:.4f}")

### SVM on boundbox dataset.
sgd_model.fit(x_train_bound_box_flattened, y_train)

y_pred = sgd_model.predict(x_train_bound_box_flattened)
bb_accuracy = accuracy_score(y_train, y_pred)

print(f"Bound Box Accuracy on train set: {bb_accuracy:.4f}")

## SVM on boundbox streched dataset.

sgd_model.fit(x_train_bound_box_streched_flattened, y_train)

y_pred = sgd_model.predict(x_train_bound_box_streched_flattened)
bbs_accuracy = accuracy_score(y_train, y_pred)

print(f"Bound Box Streched Accuracy on train set: {bbs_accuracy:.4f}")

### SVM on boundbox streched threshold dataset.
sgd_model.fit(x_train_bound_box_streched_threshold_flattened, y_train)

y_pred = sgd_model.predict(x_train_bound_box_streched_threshold_flattened)
